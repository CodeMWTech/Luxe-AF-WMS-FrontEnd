<template>
  <div class="live-page">
    ...
    <el-card class="live-filter" shadow="never"><el-form :inline="true"><el-form-item :label="tr('结算月份')"><el-date-picker v-model="query.settlementMonth" type="month" value-format="YYYY-MM" :format="LIVE_MONTH_FORMAT" clearable /></el-form-item><el-form-item :label="tr('平台')"><el-select v-model="query.platform" clearable :placeholder="tr('全部平台')"><el-option v-for="v in platforms" :key="v" :label="v" :value="v" /></el-select></el-form-item><el-form-item :label="tr('主播')"><LiveEmployeeSelect v-model="query.employeeId"   :placeholder="tr('全部主播')" :employees="options.employees" /></el-form-item><el-form-item :label="tr('状态')"><el-select v-model="query.status" clearable :placeholder="tr('全部状态')"><el-option :label="tr('正常销售')" value="NORMAL" /><el-option :label="tr('已退款')" value="REFUNDED" /></el-select></el-form-item><el-form-item><el-button type="primary" @click="load">{{ tr('查询') }}</el-button></el-form-item><el-form-item label="主播状态"><el-select v-model="query.employeeScope" @change="query.pageNum = 1; load()"><el-option label="全部" value="ALL" /><el-option label="在职/试用期" value="ACTIVE" /><el-option label="已归档" value="INACTIVE" /></el-select></el-form-item></el-form></el-card>
...
import { settlementStatusLabel, accountLabel, displayDate, displayMonth, downloadCsv, isoDate, liveEmployeeOptionLabel, LIVE_DATE_FORMAT, LIVE_MONTH_FORMAT, money } from '../shared'
...
function headers(){return[{key:'settlementMonth',label:'结算月份'},{key:'platform',label:'平台'},{key:'accountLabel',label:'直播平台'},{key:'employeeName',label:'主播'},{key:'orderNo',label:'订单号'},{key:'orderDate',label:'订单日期'},{key:'saleAmount',label:'销售额'},{key:'commissionRate',label:'佣金率'},{key:'commissionAmount',label:'佣金金额'},{key:'status',label:'状态'},{key:'employeeStatus',label:'员工状态代码'},{key:'settlementStatusLabel',label:'结算状态'},{key:'settlementId',label:'结算批次'},{key:'remark',label:'备注'}]}
async function exportRows(){const {rows: exportData}=await listCommissions({...query,pageNum:1,pageSize:2147483647});downloadCsv(`主播佣金-${query.settlementMonth||'全部'}.csv`,headers(),(exportData||[]).map(row=>({...row,settlementStatusLabel:settlementStatusLabel(row.settlementStatus),settlementMonth:displayMonth(row.settlementMonth),orderDate:displayDate(row.orderDate)})))}
