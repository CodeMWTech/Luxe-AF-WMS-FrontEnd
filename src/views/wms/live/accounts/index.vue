...
async function submit(){await formRef.value.validate();if(dialog.form.id){const existing=rows.value.find(item=>item.id===dialog.form.id),disabling=existing?.status===0&&dialog.form.status===1;try{await updateAccount(dialog.form)}catch(error){await showActionBlocked(error,disabling?dialog.form.id:null,'停用');return}}else{await addAccount(dialog.form)}proxy.$modal.msgSuccess(tr('保存成功'));dialog.open=false;load()}
