<template>
  <div class="live-page">
    <div class="live-hero"><div><h2>直播平台管理</h2><p>管理平台店铺在主播薪酬模块中的直播平台代号与显示名</p></div><div class="live-actions"><el-button @click="sync">同步授权店铺</el-button><el-button type="primary" @click="openDialog()">新增直播平台</el-button></div></div>
    ...
    <el-card class="live-card" shadow="never"><el-table v-loading="loading" :data="rows" stripe><el-table-column prop="platform" label="所属平台" /><el-table-column prop="accountCode" label="直播平台代号"><template #default="s"><el-tag class="type-tag">{{ s.row.accountCode }}</el-tag></template></el-table-column><el-table-column prop="displayName" label="显示名称" /><el-table-column prop="platformShopName" label="关联授权店铺" /><el-table-column label="状态"><template #default="s"><el-tag :type="s.row.status===0?'success':'info'">{{ tr(s.row.status===0?'启用':'停用') }}</el-tag></template></el-table-column><el-table-column prop="sortOrder" label="排序" /><el-table-column label="操作" width="140"><template #default="s"><el-button link type="primary" @click="openDialog(s.row)">{{ tr('编辑') }}</el-button><el-button link type="danger" @click="remove(s.row)">{{ tr('删除') }}</el-button></template></el-table-column></el-table><pagination v-show="total>0" :total="total" v-model:page="query.pageNum" v-model:limit="query.pageSize" @pagination="load" /></el-card>
    <el-dialog v-model="dialog.open" :title="dialog.form.id?'编辑直播平台':'新增直播平台'" width="560px"><el-form ref="formRef" :model="dialog.form" :rules="rules" label-width="100px"><el-form-item label="所属平台" prop="platform"><el-input v-model="dialog.form.platform" placeholder="TikTok / eBay / whatnot" /></el-form-item><el-form-item label="直播平台代号" prop="accountCode"><el-input v-model="dialog.form.accountCode" placeholder="如 TK 01" /></el-form-item><el-form-item label="显示名称" prop="displayName"><el-input v-model="dialog.form.displayName" /></el-form-item><el-form-item label="状态"><el-radio-group v-model="dialog.form.status"><el-radio :label="0">启用</el-radio><el-radio :label="1">停用</el-radio></el-radio-group></el-form-item><el-form-item label="排序"><el-input-number v-model="dialog.form.sortOrder" :min="0" /></el-form-item><el-form-item label="备注"><el-input v-model="dialog.form.remark" type="textarea" /></el-form-item></el-form><template #footer><el-button @click="dialog.open=false">取消</el-button><el-button type="primary" @click="submit">保存</el-button></template></el-dialog>
  </div>
</template>
<script setup>
...
async function remove(row){await proxy.$modal.confirm(`确认删除直播平台 ${row.accountCode}？历史记录中的直播平台引用将保留。`);await deleteAccount(row.id);proxy.$modal.msgSuccess('删除成功');load()}
async function sync(){const res=await syncAccounts();proxy.$modal.msgSuccess(`已新增 ${res.data||0} 个授权店铺直播平台配置`);load()}
onMounted(load)
</script>
