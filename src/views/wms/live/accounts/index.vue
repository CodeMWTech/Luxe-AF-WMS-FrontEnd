...
async function remove(row){const message=isEn.value?`Delete live platform ${row.accountCode}? Historical references will be retained.`:`确认删除直播平台 ${row.accountCode}？历史记录中的直播平台引用将保留。`;await proxy.$modal.confirm(message);try{await deleteAccount(row.id)}catch(error){await showActionBlocked(error,row.id,'删除');return}proxy.$modal.msgSuccess(tr('删除成功'));load()}
async function showActionBlocked(error,accountId,action){if(accountId){try{const res=await getAccountUsage(accountId);const usageRows=res.data||[];if(usageRows.length){Object.assign(usageDialog,{open:true,rows:usageRows,action,target:'该直播平台'});return}}catch(_){}}await proxy.$modal.alertWarning(error?.message||'操作失败，请稍后重试。')}
