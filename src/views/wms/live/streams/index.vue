<template>
  <div class="live-page">
    ...
    <el-dialog v-model="dialog.open" class="stream-entry-dialog" :title="dialog.form.id ? '编辑开播记录' : '新增开播记录'" width="820px" append-to-body>
      <el-form ref="formRef" :model="dialog.form" :rules="rules" :label-width="isEn ? '152px' : '92px'">
        <div class="dialog-grid">
          <el-form-item label="日期" prop="streamDate"><el-date-picker v-model="dialog.form.streamDate" type="date" value-format="YYYY-MM-DD" :format="isEn ? 'MM/DD/YYYY' : 'YYYY-MM-DD'" @change="handleStreamRateScopeChange" /></el-form-item>
          ...
          <el-form-item class="wide special-details-item" label="特殊明细">
            <div class="special-details">
              ...
              <div class="special-details-footer"><el-button text type="primary" @click="addSpecialDetail">+ 新增一条</el-button><span>合计：<strong>{{ money(specialTotal) }}</strong></span></div>
            </div>
          </el-form-item>
...
import { accountLabel, isoDate, money } from '../shared'
