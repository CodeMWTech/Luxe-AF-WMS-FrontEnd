<template>
  <div data-runtime-i18n-ignore="true" class="live-page">
    ...
    <el-card class="live-card settings-section" shadow="never">
      <template #header>
        <div class="settings-section__header">
          <span>{{ sectionTitle(tr('费率类型管理'), rateTypes.filter(v => v.status === ENABLE_STATUS.ENABLED).length) }}</span>
      ...
      <el-table :data="rateTypes" stripe>
        ...
        <el-table-column :label="tr('状态')">
          <template #default="s"><el-switch v-model="s.row.status" :active-value="ENABLE_STATUS.ENABLED" :inactive-value="ENABLE_STATUS.DISABLED" @change="quickSaveRateType(s.row)" /></template>
        </el-table-column>
    ...
    <el-card class="live-card settings-section" shadow="never">
      <template #header>
        <div class="settings-section__header">
          <span>{{ sectionTitle(tr('特殊金额类型管理'), specialTypes.filter(v => v.status === ENABLE_STATUS.ENABLED).length) }}</span>
      ...
      <el-table :data="specialTypes" stripe>
        ...
        <el-table-column :label="tr('状态')">
          <template #default="s"><el-switch v-model="s.row.status" :active-value="ENABLE_STATUS.ENABLED" :inactive-value="ENABLE_STATUS.DISABLED" @change="quickSaveSpecialType(s.row)" /></template>
        </el-table-column>
        <el-table-column :label="tr('操作')" width="130">
          <template #default="s">
    ...
    <el-dialog data-runtime-i18n-ignore="true" v-model="rateDialog.open" class="settings-dialog" :title="tr(rateDialog.form.id ? '编辑费率类型' : '新增费率类型')" width="520px">
      <el-form :model="rateDialog.form" :label-width="isEn ? '120px' : '90px'">
        ...
        <el-form-item :label="tr('状态')">
          <el-radio-group v-model="rateDialog.form.status"><el-radio :label="ENABLE_STATUS.ENABLED">{{ tr('启用') }}</el-radio><el-radio :label="ENABLE_STATUS.DISABLED">{{ tr('停用') }}</el-radio></el-radio-group>
        </el-form-item>
      </el-form>
    ...
    <el-dialog data-runtime-i18n-ignore="true" v-model="specialDialog.open" class="settings-dialog" :title="tr(specialDialog.form.id ? '编辑特殊金额类型' : '新增特殊金额类型')" width="500px">
      <el-form :model="specialDialog.form" :label-width="isEn ? '120px' : '90px'">
        ...
        <el-form-item :label="tr('状态')"><el-radio-group v-model="specialDialog.form.status"><el-radio :label="ENABLE_STATUS.ENABLED">{{ tr('启用') }}</el-radio><el-radio :label="ENABLE_STATUS.DISABLED">{{ tr('停用') }}</el-radio></el-radio-group></el-form-item>
      </el-form>
...
function openRateType(row={}){rateDialog.form={id:row.id,typeName:row.typeName||'',description:row.description||'',status:row.status??ENABLE_STATUS.ENABLED,sortOrder:row.sortOrder||rateTypes.value.length*10+10};rateDialog.open=true}
...
async function quickSaveRateType(row){try{await updateRateType(row);proxy.$modal.msgSuccess(tr('状态已更新'))}catch(error){await load();throw error}}
...
function openSpecialType(row={}){specialDialog.form={id:row.id,typeName:row.typeName||'',category:row.category||'OTHER',status:row.status??ENABLE_STATUS.ENABLED,sortOrder:row.sortOrder||specialTypes.value.length*10+10};specialDialog.open=true}
...
async function quickSaveSpecialType(row){try{await updateSpecialType(row);proxy.$modal.msgSuccess(tr('状态已更新'))}catch(error){await load();throw error}}
