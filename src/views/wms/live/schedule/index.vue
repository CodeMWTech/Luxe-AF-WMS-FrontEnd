...
const rules = { scheduleDate: [{ required: true, message: '请选择日期' }], employeeId: [{ required: true, message: '请选择主播' }], accountId: [{ required: true, message: '请选择账号' }], rateTypeId: [{ required: true, message: '请选择场次类型' }], startTime: [{ required: true, message: '请选择开始时间' }], endTime: [{ validator: validateEndTime, trigger: 'change' }] }
...
function openDialog(row = {}) { dialog.form = { id: row.id, employeeName: row.employeeName || '', scheduleDate: row.scheduleDate || defaultScheduleDate(), employeeId: row.employeeId || null, accountId: row.accountId || null, rateTypeId: row.rateTypeId || null, startTime: row.startTime || '09:00:00', endTime: row.endTime || '17:00:00', remark: row.remark || '' }; dialog.open = true }
